...
async fn main() -> Result<()> {
    ...
    println!("MCP Agent with OpenAI GPT Demo\n");
    println!("This agent will use tools to answer your questions.\n");

    // Create a tool handler with the same 8 tools as server_with_tools.rs
    ...
    let openai = OpenAIAdapter::from_env("gpt-5".to_string())
        .map_err(|e| {
            eprintln!("Error: Failed to initialize OpenAI adapter. Make sure OPENAI_API_KEY is set.");
            eprintln!("Details: {}", e);
            e
        })?;
    ...
    let _server_task = tokio::spawn(async move {
        ...
        // MCP-compatible tools list endpoint for OpenAI Responses API
        ...
        // MCP-compatible tool call endpoint for OpenAI Responses API
        async fn handle_call_tool(
            State(state): State<ServerState>,
            Json(request): Json<serde_json::Value>,
        ) -> Json<serde_json::Value> {
            let tool_name = request.get("tool").and_then(|v| v.as_str()).unwrap_or("");
            let tool_input = request.get("input").cloned().unwrap_or(serde_json::json!({}));

            match state.server.handle_tool_call(tool_name, tool_input).await {
        ...
        let router = Router::new()
            ...
            .route("/mcp/list-tools", post(handle_list_tools))
    ...
    // Set the MCP client on the OpenAI adapter so it can execute tools
    let openai_with_client = openai.with_mcp_client(std::sync::Arc::new(client.clone()));

    // Create the agent
    let mut agent = Agent::new(client, Arc::new(openai_with_client), agent_config);
    ...
    println!("Running demo queries with OpenAI GPT-5 agent:\n");
